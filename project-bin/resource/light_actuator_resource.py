...
class LightActuatorResource(resource.Resource): #se cambio qualcosa, viene rimandata la risorsa nuova
    ...
    def __init__(self,device_name):
        ...
        self.device_info = LightSmartObj(room=1,light_id=2) #?????????
       # self.state= #devo metterci uno stato passato? O uno stato comunque
        self.if_= "core.a"
    ...
    async def render_put(self, request):
        print('LightActuatorResource -> PUT Byte payload: %s' % request.payload)
        json_payload_string = request.payload.decode('UTF-8')
        print('LightActuatorResource -> PUT String Payload: %s' % json_payload_string)
        change_light_request = LightRequestDescriptor(**json.loads(json_payload_string))
        print('Change Light Request Received:%s' % change_light_request.type)
